...
def plot_mil_exp_x_gdp_pcap(year):
    ...
    # Remover valores não numéricos ou nulos
    df_gdp_pcap_viz = df_gdp_pcap_viz[pd.to_numeric(df_gdp_pcap_viz[str(year)], errors='coerce').notnull()]
    ...
    df_gdp_pcap_viz = df_gdp_pcap_viz[df_gdp_pcap_viz['country'].isin(df_mil_exp_viz['country'])]
    df_mil_exp_viz = df_mil_exp_viz[df_mil_exp_viz['country'].isin(df_gdp_pcap_viz['country'])]
    ...
    with col1:
        st.subheader(f"Military Expenditure ({year})")
